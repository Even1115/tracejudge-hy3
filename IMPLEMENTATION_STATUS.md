...
- CLI：`doctor` / `demo --mock --case {correct,faulty}` / `dataset convert-humanevalplus` / `dataset sample`（支持 `--selection-role pilot|research_natural` 和 `--exclude-manifest`） / `dataset validate` / `baseline` / `evalplus`（支持 `--selection-policy all|phase1-success-only` 与 `--min-success-count`） / `run` / `batch`。`baseline` 提供 `--dataset-manifest`，HumanEval+ 公开投影必须传入匹配的 manifest，且会在 Provider 调用前验证 provenance；`run` / `batch` 明确拒绝该阶段一投影，阶段二只能通过独立的 `evalplus`。
