...
**当前录制流程见 [全流程分镜与操作文档](full_flow_storyboard.md)。**
`?recording=1` 现为八幕手动展示，支持上一幕／下一幕与方向键；本次执行完成后才开放结果画面。以下保留运行模式与数据来源说明，普通工作台仍支持结果自动展开。
...
- 录制模式：<http://127.0.0.1:8765/?recording=1>（逐幕展示、来源标识与手动控制）
- Windows：在仓库根目录运行 `.\scripts\run_recording_demo.ps1`，自动检查并按需启动 Docker Desktop，等待引擎就绪后启动演示服务；离线预演加 `-Offline` 可跳过 Docker。默认等待 Docker 120 秒，可用 `-DockerWaitSeconds 240` 延长。
...
4. 运行完成后用“下一幕”依次展示：结构化解答（含步骤与代码）→
   沙盒测试执行（可见通过、`[]` 触发 ZeroDivisionError）→ 四层对齐与静态证据 → 首错定位
   （`S1` / `R1` / `A01_PLAN_CODE_MISMATCH`）→ 最小反例 `[[]]`、
   `confirmed_bug` 证书与重放复现结果 → 结果文件相对路径与耗时。
5. 第 7 幕展示独立公开 reasoning_swap 对照；第 8 幕展示“已发布结果总览”。数字直接读取已核验的公开报告，含方法名、分子分母和复标状态，不手工沿用旧版字幕。
6. 停止录制。按新版分镜剪辑至 118 秒以内；实际运行耗时另行保留。
