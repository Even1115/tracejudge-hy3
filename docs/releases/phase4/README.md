...
- [`phase4_contest_results_overview_v1.md`](phase4_contest_results_overview_v1.md)：面向评审的一页结果总览，独立展示四个主实验核心数字、五方法 TP/FP/TN/FN、FPR，以及第二标注者 20 条子集的聚合一致性和结果边界。
...
- [`phase4_judge_stability_sensitivity_v1.md`](phase4_judge_stability_sensitivity_v1.md)：从正式 4 × 5 Hy3 Judge 稳定性 run 离线生成的独立敏感性报告；保留首错步骤原始精确一致率 90.0%，并将精确别名规范化后的 100.0% 明确标为 post-hoc。对应机器可读聚合为 [`phase4_judge_stability_sensitivity_v1.json`](phase4_judge_stability_sensitivity_v1.json)。
- [`phase4_p1_post_adjudication_sensitivity_v1.md`](phase4_p1_post_adjudication_sensitivity_v1.md)：唯一过程细节分歧完成共识裁决后的 aggregate-only、post-hoc 敏感性分析；保留原始完整记录 19/20 和 `has_error` 20/20，记录 1/1 分歧已解决，并报告下游指标零影响或最多一条样本的变化上界，不生成新的 20/20 标注者一致率。对应机器可读聚合为 [`phase4_p1_post_adjudication_sensitivity_v1.json`](phase4_p1_post_adjudication_sensitivity_v1.json)，JSON / Markdown SHA256 为 `377725050f8adbb4afe88f0b0e01ae05b4a2bc670c6920034fc8bb5b0472a48b` / `7dd2f1f244c3bd09a2928b61c1ee36cb25e59a88e8631e0be3807d504384866d`。
- [`charts/contest_showcase_v1/05_judge_stability_card.svg`](charts/contest_showcase_v1/05_judge_stability_card.svg)：面向首页的稳定性结果卡片，展示 20/20 有效判断、零 Provider/解析失败以及原始与 post-hoc 指标边界；不并入三张冻结正式图表。
