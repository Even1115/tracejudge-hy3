...
| Gate E：Demo、图表与 Release 封版 | 仓库内交付完成 | 3 张确定性聚合 SVG、2 分钟公开 Fixture Demo、Release 检查单和本封版报告已就绪；Pull Request #4 已通过 merge commit `9627d93b668891c1fba0b255e403168afa731bf1` 合并到 `main`；tag、Release 和附件上传待授权。 |
...
本轮封版验证结果：Gate E focused tests 与完整普通测试均通过，完整测试中的 3 项 skipped 均为未启用的 Docker 集成检查；Ruff 检查、Ruff 格式检查和图表逐字节验证通过。原工作树级 `git diff --check` 不作为最终提交链证据；最终检查改为对 `origin/main...HEAD` 提交范围执行，并明确排除必须保持 Gate F 逐字节身份的公开报告。该报告改用固定 SHA256 和与 Git-ignored 冻结源的 `cmp` 单独验证。测试和图表验证没有调用真实 Hy3、Docker 或网络。
...
本报告完成的是仓库内 P0 Gate E 交付，不代表已完成外部发布。仓库内审查和提交已经完成；[Pull Request #4](https://github.com/Even1115/tracejudge-hy3/pull/4) 已通过 merge commit `9627d93b668891c1fba0b255e403168afa731bf1` 合并到 `main`；当前未创建 tag、GitHub Release 或上传附件。
